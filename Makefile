...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "complexion", "tests")); system.time(test_check("complexion", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran complexion_$(VERSION).tar.gz
	rm -rf complexion.Rcheck/
